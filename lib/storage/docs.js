...
bedrock.events.on('bedrock-mongodb.ready', async () => {
  ...
  if(DOC_COMPATIBILITY_VERSION !== 1) {
    throw new Error('"documentCompatibilityVersion" must be 1.');
  }
  ...
  await database.createIndexes([{
    ...
    collection: COLLECTION_NAME,
    fields: {
      // FIXME: adding `doc.id` to this index will allow queries that return
      // only doc IDs to be completely index-covered queries
      localEdvId: 1,
      attributes: 1
    },
    options: {
...
/**
 * Builds a database query to be passed to `find` or `count` using the given
 * EDV index parameters. The `index`, `equals`, and `has` parameters correspond
 * to the same-named fields of the @interop/data-integrity-core IEDVQuery
 * interface.
 *
 * @param {object} options - The options to use.
 * @param {string} options.index - The EDV index ID (the HMAC ID).
 * @param {Array<object>} [options.equals] - The EDV `equals` query array; each
 *   element is an object of blinded `name: value` pairs.
 * @param {Array<string>} [options.has] - The EDV `has` query array of blinded
 *   attribute names.
 *
 * @returns {object} The database query.
 */
 ...
 * @param {IEncryptedDocument} options.doc - The encrypted document to insert.
 *
...
/**
 * Counts the EDV documents that match the given query.
 *
 * @param {object} options - The options to use.
 * @param {string} options.edvId - The ID of the EDV to query.
 * @param {object} [options.query={}] - The query to use (default: {}).
 * @param {object} [options.options={}] - Query options (eg: 'sort', 'limit').
 * @param {boolean} [options.explain=false] - An optional explain boolean.
 *
 * @returns {Promise<number | ExplainObject>} Resolves with the number of
 *   matching documents or an ExplainObject if `explain=true`.
 */
 ...
 * @param {IEncryptedDocument} options.doc - The encrypted document to store.
 * @param {boolean} [options.explain=false] - An optional explain boolean.
 *
...
export async function _handleDocumentCompatibility({checkIndexPromise} = {}) {
  ...
  try {
    ...
    // ignore `IndexNotFound` error, log any other error but indicate that
    // the index might have already be dropped concurrently and that this
    // will be checked again later w/o issue
...
/**
 * The encrypted, server-stored form of an EDV document (not the decrypted
 * IEDVDocument).
 *
 * @typedef {
 *   import('@interop/data-integrity-core').IEncryptedDocument
 * } IEncryptedDocument
 */
